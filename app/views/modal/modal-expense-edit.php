        ...
        <form class="input-container" method="POST" action="record-expense" enctype="multipart/form-data">
            ...
            <select name="budget_account_id" required>
                ...
                <?php foreach ($budgetAccounts as $account): ?>
                    <option value="<?php echo $account["id"]; ?>" <?php echo ($expenseData["budget_account_id"] == $account["id"]) ? "selected" : ""; ?>>
            ...
            <p>Volume</p>
            <input type="number" name="volume" placeholder="Contoh: 1" value="<?php echo $expenseData["volume"] ?? ""; ?>" required>

            <p>Harga Satuan</p>
            <div class="input-group">
                <span class="input-prefix">Rp</span>
                <input type="number" name="unit_price" placeholder="Harga" value="<?php echo $expenseData["unit_price"] ?? ""; ?>" required>
            </div>
            ...
            <div class="input-group">
                ...
                <input type="text" placeholder="<?php echo !empty($expenseData["proof"]) ? htmlspecialchars($expenseData["proof"]) : "No file chosen"; ?>" readonly>
            </div>

            <p>Deskripsi</p>
            <textarea name="description" rows="4"><?php echo htmlspecialchars($expenseData["description"]) ?? ""; ?></textarea>

            <input type="hidden" name="item_id" value="<?php echo $expenseData["id"] ?? ""; ?>">
